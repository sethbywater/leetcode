...
/// Uses a custom Queue type for sorted insertions
/// TODO Not working
pub fn trap_rain_water(height_map: Vec<Vec<i32>>) -> i32 {
	...
	// Load edges
	for (i, h) in height_map[0].iter().enumerate() { heap.push((Reverse(*h), 0, i)) }
	for i in 1..n-1 {
		heap.push((Reverse(height_map[i][0]), i, 0));
		heap.push((Reverse(height_map[i][m-1]), i, m-1));
	}
	for (i, h) in height_map[n-1].iter().enumerate() { heap.push((Reverse(*h), n-1, i)) }
